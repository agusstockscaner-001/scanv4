...
def backtest_symbol(symbol, target_pct=5, sl_pct=3, hold_days=6):
  try:
    ...
    for i in range(60, n-hold_days, 2):  # step 2 hari
        ...
        resist20 = max(hi[i-20:i])
        is_breakout = cl[i] > resist20 and avgv>0 and vol[i] > avgv*1.5
        smart = is_accum or is_breakout
        # weekly trend sederhana (tren 20 hari naik)
        wk_bull = cl[i] > cl[i-20] if i>=20 else False
        # R/R — support dekat (10 hari), target = resistance terdekat atau proyeksi 8%
        sup = min(lo[i-10:i])
        res = max(hi[i-20:i])
        if res <= cl[i]*1.02: res = cl[i]*1.08  # kalau sudah dekat/lewat resistance, proyeksi target +8%
        risk = cl[i]-sup
        rr = (res-cl[i])/risk if risk>0 else 0
        # anti-FOMO DILONGGARKAN: 6% -> 12%
        not_fomo = (cl[i]-e20)/e20*100 <= 12
        # sinyal DILONGGARKAN: RSI 40-72, volume 1.15x, smart money ATAU weekly bull
        sig = (cl[i]>e20>e50 and 40<=rv<=72 and ml>ms and vr>=1.15
               and (smart or wk_bull) and rr>=1.2 and not_fomo)
        if sig:
            entry = cl[i]
